...
// handle printing of a pointer value
int	handle_pointer(uintptr_t n)
{
	int	count;

	// if the pointer is null, print "(nil)" and return 5 (because (nil) has 5 chars)
	if (n == 0)
	{
		write(1, "(nil)", 5);
		return (5);
	}
	write(1, "0x", 2); // print the "0x" prefix for hexadecimal representation
	count = count_hex_digits(n); // get the number of hex digits in the pointer
	return (count + 2); // returns the number of characters that were output
}
